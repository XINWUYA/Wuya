...
void CalculateMaterial(inout SMaterialInput mtl)
{
	...
	mtl.Albedo = SrgbToLinear(texture(u_AlbedoTexture, Input.TexCoord).rgb);

	vec3 T = normalize(Input.Tangent);
	vec3 B = normalize(Input.BiTangent);//cross(Input.Normal, Input.Tangent.xyz);
	vec3 N = normalize(Input.Normal);
	mat3 TBN = mat3(T, B, N);
	
	vec3 normal = texture(u_NormalTexture, Input.TexCoord).xyz;
	mtl.Normal = TBN * normalize(normal * 2.0f - vec3(1.0f));
