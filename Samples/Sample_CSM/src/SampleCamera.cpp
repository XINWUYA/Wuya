...
namespace Wuya
{
	...
	void SampleCamera::ConstructRenderView()
	{
		...
		auto shadow_cast_pass = frame_graph->AddPass<ShadowCastData>("ShadowCastPass",
			[&](FrameGraphBuilder& builder, ShadowCastData& data)
			{
				
			},
			[&](const FrameGraphResources& resources, const ShadowCastData& data)
			{
				Renderer::GetRenderAPI()->PushDebugGroup("ShadowCastPass");
